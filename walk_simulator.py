...
def extract_coordinates_with_time(gpx_data):
    ...
    for idx, point in enumerate(track_points):
        ...
        coordinates.append(
            {"lat": lat, "lon": lon, "elapsed_time": elapsed_time, "speed": speed}
        )
...
def get_video_instructions_from_coordinates(coordinates, valhalla_instructions):
    ...
    for coordinate in coordinates:
        ...
        action_coordinate = (
            instruction_info["location"][0],
            instruction_info["location"][1],
        )

        distance = haversine(
            current_gps_coordinate, action_coordinate, unit=Unit.METERS
        )
